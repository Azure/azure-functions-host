...
namespace Microsoft.Extensions.DependencyInjection
{
    public static class ScriptJwtBearerExtensions
    {
        ...
        private static bool TryGetNormalizedSiteName(string appName, out string normalizedSiteName)
        {
            normalizedSiteName = null;

            int idx = appName.IndexOf(SlotSeparator);
            if (idx != -1)
            {
                normalizedSiteName = appName.Substring(0, idx);
                return true;
            }
