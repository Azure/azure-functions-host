        ...
        public static string Decrypt(byte[] encryptionKey, string value)
        {
            ...
            using (var aes = new AesManaged { Key = encryptionKey })
            {
                using (var ms = new MemoryStream())
                {
                    ...
                    {
                        throw new InvalidOperationException($"Signature mismatches! Signature Received: {Convert.ToBase64String(signature)} Expected: {Convert.ToBase64String(ComputeHMACSHA256(encryptionKey, input))} input: {Encoding.UTF8.GetString(input)}");
                    }
