            ...
            await Utility.InvokeWithRetriesAsync(async () =>
            {
                try
                {
                    using (_metricsLogger.LatencyEvent(downloadMetricName))
                    {
                        ...
                        // CodeQL [SM03781] - The URL is from a trusted source for blob storage and file shares. Also, an exception was granted by the CodeQL team for this open-source repo.
        ...
        private async Task<bool> IsResourceAccessibleWithoutAuthorization(string resourceUrl)
        {
            try
            {
                using (var cts = new CancellationTokenSource())
                {
                    ...
                    {
                        // CodeQL [SM03781] - The URL is from a trusted source for blob storage and file shares and the call only checks for status code. Also granted exception from CodeQL team regarding this open-source repo.
