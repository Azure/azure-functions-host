...
{
    public class AtlasAssignmentHandler : IAssignmentHandler
    {
        ...
        private readonly ILogger<AtlasAssignmentHandler> _logger;

        public AtlasAssignmentHandler(IOptionsFactory<ScriptApplicationHostOptions> optionsFactory,
            IEnvironment environment, IRunFromPackageHandler runFromPackageHandler, IPackageDownloadHandler packageDownloadHandler,
            IHttpClientFactory httpClientFactory, IMeshServiceClient meshServiceClient, IMetricsLogger metricsLogger,
            ILogger<AtlasAssignmentHandler> logger)
        {
        ...
        public async Task<string> Download(HostAssignmentContext assignmentContext)
        {
            // Based on profiling download code jit-ing holds up cold start.
            // Pre-jit to avoid paying the cost later.
            return await _packageDownloadHandler.Download(assignmentContext.GetRunFromPkgContext());
        }

        public async Task ApplyFileSystemChanges(HostAssignmentContext assignmentContext)
        {
