...
namespace Microsoft.Azure.WebJobs.Script.WebHost.Diagnostics
{
    internal class LinuxContainerEventGenerator : LinuxEventGenerator
    {
        ...
        public static string AzureMonitoringEventRegex { get; } = $"{ScriptConstants.LinuxAzureMonitoringEventStreamName} (?<Level>[0-6]),(?<ResourceId>[^,]*),(?<OperationName>[^,]*),(?<Category>[^,]*),(?<RegionName>[^,]*),(?<Properties>[^,]*)";
        ...
        {
            _writeEvent($"{ScriptConstants.LinuxAzureMonitoringEventStreamName} {(int)ToEventLevel(level)},{resourceId},{operationName},{category},{regionName},{properties}");
        }
    }
}