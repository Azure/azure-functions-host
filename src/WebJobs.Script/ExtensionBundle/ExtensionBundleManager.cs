...
namespace Microsoft.Azure.WebJobs.Script.ExtensionBundle
{
    public class ExtensionBundleManager : IExtensionBundleManager
    {
        ...
        private async Task<string> DownloadExtensionBundleAsync(string version, HttpClient httpClient)
        {
            ...
            var zipUri = new Uri($"{_cdnUri}/{ScriptConstants.ExtensionBundleDirectory}/{_options.Value.Id}/{version}/{version}.zip");
        ...
        {
            var uri = new Uri($"{_cdnUri}/{ScriptConstants.ExtensionBundleDirectory}/{_options.Value.Id}/{ScriptConstants.ExtensionBundleVersionIndexFile}");
