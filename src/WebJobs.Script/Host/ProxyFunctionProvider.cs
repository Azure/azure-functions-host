...
namespace Microsoft.Azure.WebJobs.Script
{
    public class ProxyFunctionProvider : IFunctionProvider, IDisposable
    {
        ...
        internal static Collection<ProxyFunctionMetadata> ReadProxyMetadata(string scriptPath, ILogger logger, ILogger loggerRequests, Dictionary<string, ICollection<string>> functionErrors = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(proxiesJson))
            {
                ...
                var metadataCollection = LoadProxyMetadata(proxiesJson, functionErrors, logger, loggerRequests);
        ...
        private static Collection<ProxyFunctionMetadata> LoadProxyMetadata(string proxiesJson, Dictionary<string, ICollection<string>> functionErrors, ILogger logger, ILogger loggerRequests)
        {
            var proxies = new Collection<ProxyFunctionMetadata>();
            ProxyClientExecutor client = null;

            var rawProxyClient = ProxyClientFactory.Create(proxiesJson, logger, loggerRequests);
