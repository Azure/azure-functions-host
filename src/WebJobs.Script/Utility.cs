...
namespace Microsoft.Azure.WebJobs.Script
{
    public static class Utility
    {
        ...
        {
            // External worker mode always uses worker-provided metadata.
            string externalEnabled = environment.GetEnvironmentVariable(EnvironmentSettingNames.FunctionsWorkerExternalEnabled);
            if (string.Equals(externalEnabled, "true", StringComparison.OrdinalIgnoreCase)
                || string.Equals(externalEnabled, "1", StringComparison.Ordinal))
            {
                return true;
            }
