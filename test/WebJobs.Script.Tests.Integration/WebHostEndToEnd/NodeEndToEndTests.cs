...
//            var metadata = new Dictionary<string, string> { { "TestMetadataKey", "TestMetadataValue" } };
//            await inputBlob.SetMetadataAsync(metadata);
...
//            var data = (JObject)blobMetadata["metadata"];
//            Assert.Equal("TestMetadataValue", (string)data["testMetadataKey"]);

//            var properties = (JObject)data["properties"];
//            Assert.Equal("application/octet-stream", (string)properties["contentType"]);
//            var blobProperties = await resultBlob.GetPropertiesAsync();
//            Assert.Equal("BlockBlob", blobProperties.Value.BlobType.ToString());
...
//            string base64Message = Convert.ToBase64String(inputBytes);
...
//                    int blobInt = BitConverter.ToInt32(blobData, 0);
...
//            var blobString = await blobs.Single().DownloadContentAsync();
