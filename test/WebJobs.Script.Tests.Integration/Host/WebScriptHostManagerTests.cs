    ...
    public class WebScriptHostManagerTests : IClassFixture<WebScriptHostManagerTests.Fixture>, IDisposable
    {
        ...
        public class Fixture : IDisposable
        {
            ...
            public Fixture()
            {
                ...
                {
                    Assert.True(EventGenerator.SystemEvents.Any(p => Regex.IsMatch(p, pattern)), $"Expected trace event '{pattern}' not found.");
                }
            }
            ...
            public class TestSystemEventGenerator : IEventGenerator
            {
                ...
                {
                    var elements = new string[] { level.ToString(), subscriptionId, appName, functionName, eventName, source, summary, details, exceptionType, exceptionMessage, functionInvocationId, hostInstanceId, activityId };
