...
namespace Microsoft.Azure.WebJobs.Script.Tests.Workers.Rpc
{
    public class WorkerConfigurationProviderBaseTests
    {
        ...
        public void DefaultLanguageWorkersDir()
        {
            ...
            var optionsMonitor = GetTestWorkerConfigurationResolverOptions(config, workerRuntime: null, testEnvironment, testScriptHostManager.Object, null);
        ...
        public void GetDefaultWorkersDirectory_Returns_Expected(bool expectedValue)
        {
            ...
            Mock<IWorkerRuntimeResolver> mockRuntimeResolver = new Mock<IWorkerRuntimeResolver>();
            mockRuntimeResolver.Setup(r => r.GetWorkerRuntime(It.IsAny<string>())).Returns("node");
            var optionsSetup = new WorkerConfigurationResolverOptionsSetup(
                loggerFactory,
                config,
                env.Object,
        ...
        public void WorkerDescription_Skipped_When_Profile_Disables_Worker()
        {
            ...
            var loggerFactory = GetTestLoggerFactory();
            var testScriptHostManager = new Mock<IScriptHostManager>();
            var optionsMonitor = GetTestWorkerConfigurationResolverOptions(config, workerRuntime: null, testEnvironment, testScriptHostManager.Object);
            var dynamicProviderLogger = new TestLogger<DynamicWorkerConfigurationProvider>();
            var providers = GetProviders(testLoggerFactory, dynamicProviderLogger, testMetricLogger, FileUtility.Instance, testProfileManager.Object, TestProcessFacts.Current, optionsMonitor);
        ...
        public void DefaultWorkerConfigs_Overrides_DefaultWorkerRuntimeVersion_AppSetting(string setting, string output)
        {
            ...
            var workerProfileManager = new WorkerProfileManager(
                workerProfileLogger, testEnvironment, TestProcessFacts.Current);
        ...
        public void DefaultWorkerConfigs_Overrides_VersionAppSetting(string setting, string output)
        {
            ...
            var testLogger = loggerFactory.CreateLogger("test");
            var testScriptHostManager = new Mock<IScriptHostManager>();
            var optionsMonitor = GetTestWorkerConfigurationResolverOptions(config, "powerShell", testEnvironment, testScriptHostManager.Object, null);
            var dynamicProviderLogger = new TestLogger<DynamicWorkerConfigurationProvider>();
        ...
        public void GetWorkerProcessCount_Tests(bool defaultWorkerConfig, bool setProcessCountToNumberOfCpuCores, bool setWorkerCountInEnv, int minProcessCount, int maxProcessCount, string processStartupInterval)
        {
            ...
            var providers = GetProviders(mockLogger.Object, dynamicProviderLogger, testMetricLogger, FileUtility.Instance, testProfileManager.Object, TestProcessFacts.Current, optionsMonitor);

            var workerConfigurationResolver = new WorkerConfigurationResolver(providers);

            var result = WorkerConfigurationProviderBase.GetWorkerProcessCount(
                workerConfig,
                testEnvironment.GetEnvironmentVariable(RpcWorkerConstants.FunctionsWorkerProcessCountSettingName),
                testEnvironment.GetEffectiveCoresCount());
