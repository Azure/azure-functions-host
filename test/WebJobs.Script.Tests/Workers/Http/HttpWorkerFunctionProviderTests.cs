...
namespace Microsoft.Azure.WebJobs.Script.Tests.Workers.Http
{
    public class HttpWorkerFunctionProviderTests
    {
        ...
        [Fact]
        public async Task GetFunctionMetadataAsync_ConfiguredRoutes_CurrentImplementationThrows_IndexIssue()
        {
            // NOTE: The current implementation iterates i=1..Count and uses ElementAt(i),
            // which will throw for any non-empty route list (off-by-one). This test captures
            // current behavior so a later fix can adjust the expectation.
            var hostMeta = new Mock<IHostFunctionMetadataProvider>(MockBehavior.Strict);
            hostMeta.Setup(m => m.GetFunctionMetadataAsync(It.IsAny<IEnumerable<RpcWorkerConfig>>(), false))
            ...
            var routes = new[]
            {
                ...
                new HttpWorkerRoute("/two", AuthorizationLevel.Function)
            };
            ...
            await Assert.ThrowsAnyAsync<Exception>(() => provider.GetFunctionMetadataAsync());
