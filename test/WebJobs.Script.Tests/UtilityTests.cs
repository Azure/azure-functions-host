...
namespace Microsoft.Azure.WebJobs.Script.Tests
{
    ...
    public class UtilityTests
    {
        ...
        [Fact]
        public void Test_LogAutorestGeneratedJson_Without_AutorestGeneratedJson()
        {
            ...
            Utility.LogAutorestGeneratedJson(functionAppDirectory, _testLogger);
            var allLogs = _testLogger.GetLogMessages();
            Assert.Empty(allLogs);
        }

        [Fact]
        public void Test_LogAutorestGeneratedJson_With_Valid_AutorestGeneratedJson()
        {
            string GetTemporaryDirectory()
            {
                string tempDirectory = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
            ...
            string functionAppDirectory = GetTemporaryDirectory();
            string sample_autorest_generated_json = @"{
  ""name"": ""@autorest/azure-functions-csharp"",
  ""version"": ""0.2.0-preview""
}
";
            string expected_contents = @"autorest_generated.json file found generated by Autorest (https://aka.ms/stencil) | file content";
            File.WriteAllText(Path.Combine(functionAppDirectory, ScriptConstants.AutorestGeenratedMetadataFileName), sample_autorest_generated_json);
            Utility.LogAutorestGeneratedJson(functionAppDirectory, _testLogger);
            var allLogs = _testLogger.GetLogMessages();
            VerifyLogLevel(allLogs, expected_contents, LogLevel.Information);
        }

        [Fact]
        public void Test_LogAutorestGeneratedJson_With_Invalid_AutorestGeneratedJson()
        {
            string GetTemporaryDirectory()
            {
                string tempDirectory = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
...
";
            string expected_contents = @"autorest_generated.json file found is incorrect (https://aka.ms/stencil) | exception";
            File.WriteAllText(Path.Combine(functionAppDirectory, ScriptConstants.AutorestGeenratedMetadataFileName), sample_autorest_generated_json);
            Utility.LogAutorestGeneratedJson(functionAppDirectory, _testLogger);
            var allLogs = _testLogger.GetLogMessages();
            VerifyLogLevel(allLogs, expected_contents, LogLevel.Information);
