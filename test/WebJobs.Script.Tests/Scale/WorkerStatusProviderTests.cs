...
namespace Microsoft.Azure.WebJobs.Script.Tests.Scale
{
    public class WorkerStatusProviderTests
    {
        ...
        public WorkerStatusProviderTests()
        {
            ...
            _workerStatusProvider = new WorkerStatusProvider(_mockPerformanceManager.Object, _traceWriter);
        }

        [Fact]
        public async Task GetWorkerStatus_LowLoad_ReturnsExpectedResult()
        {
            _mockPerformanceManager.Setup(p => p.IsUnderHighLoad(It.IsAny<Collection<string>>())).Returns(false);
        ...
        {
            _mockPerformanceManager.Setup(p => p.IsUnderHighLoad(It.IsAny<Collection<string>>())).Returns(true)
