...
- `Executing HTTP request` no longer logs the user agent or request URI, `Executed HTTP request` now logs the matched route template instead, `Sanitizer` redacts email addresses, and trace context attribute values are no longer logged. (#12039)
